...
- Multi-targeted projects appear once per TFM. `find_*`/`search`/`call_hierarchy`/`find_unused` dedupe by `SymbolFormat.IdentityKey` — assembly-simple-name + declaration file+span for source, assembly identity + doc-id for metadata — so genuine duplicates (a symbol seen once per TFM) collapse while two projects that share an FQN, or a linked/shared file compiled into two assemblies, stay distinct and resolve to a teaching ambiguity error. Symbol resolution (fuzzy, metadata, and the doc-id fast path), file/project/solution diagnostics, `analyze_impact`, and `find_unused` all iterate **every** TFM flavor; diagnostic deltas dedupe by id+path+message (line-insensitive, so a benign line shift is not a false break). Project names shown as `Name (net8.0)`.
...
- Symbol identity for dedupe/comparison is `SymbolFormat.IdentityKey` (above); `GetDocumentationCommentId` is only the *re-query currency* an agent pastes back — a doc-id can name more than one declaration (same FQN in two projects), so the doc-id resolver collects all matches, dedupes by IdentityKey, and raises the same ambiguity error rather than picking one.
